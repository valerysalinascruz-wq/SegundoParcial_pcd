...
total_transacciones = todos_montos.size

ids_iqr_global = set()
ids_zscore_global = set()
for cat in categorias:
    ids_iqr_global |= set(outliers_iqr[cat]['ids'])
    ids_zscore_global |= set(outliers_zscore[cat]['ids'])

total_outliers_unicos = len(ids_iqr_global | ids_zscore_global)
